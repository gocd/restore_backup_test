...
task :restore do
  ...
  %w{db.zip config-repo.zip config-dir.zip}.each{|f|
    snapshot = JSON.parse("#{BACKUP_DOWNLOAD_FOLDER}/snapshot.json")
    assert snapshot[:MD5][f.to_sym] == Digest::MD5.hexdigest(File.read "#{BACKUP_DOWNLOAD_FOLDER}/#{f}")
  }
  Redhat.new.install("go-server-#{server_version}")
  {"db.zip" => "/var/lib/go-server/db/h2db/",  "config-dir.zip" => "/etc/go/config/","config-repo.zip" => "/var/lib/go-server/db/config.git/"}.each do |file, dest|
    restore_files("#{BACKUP_DOWNLOAD_FOLDER}/#{file}", dest)
  end

  DB = Sequel.connect("jdbc:h2:file:/var/lib/go-server/db/h2db/cruise;user=sa;password=''")
  DB["SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='TABLE'"].each{|t|
    table = t[:table_name]
    snapshot = JSON.parse("#{BACKUP_DOWNLOAD_FOLDER}/snapshot.json")
    assert snapshot[:TABLES][table.to_sym] == DB[table.to_sym].count
  }
...
task :backup_snapshot do
  ...
  DB = Sequel.connect("jdbc:h2:file:#{Dir.tmpdir}/db/cruise;user=sa;password=''")
  DB["SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE='TABLE'"].each{|t|
    table = t[:table_name]
